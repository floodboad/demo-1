...
-- 导出 disconf 的数据库结构
CREATE DATABASE IF NOT EXISTS `disconf` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `disconf`;
...
-- 正在导出表  disconf.app 的数据：~1 rows (大约)
DELETE FROM `app`;
...
-- 正在导出表  disconf.config 的数据：~16 rows (大约)
DELETE FROM `config`;
...
-- 导出  表 disconf.config_history 结构
...
-- 正在导出表  disconf.config_history 的数据：~0 rows (大约)
...
-- 正在导出表  disconf.env 的数据：~4 rows (大约)
DELETE FROM `env`;
...
-- 正在导出表  disconf.role 的数据：~3 rows (大约)
DELETE FROM `role`;
...
-- 正在导出表  disconf.role_resource 的数据：~75 rows (大约)
...
-- 正在导出表  disconf.user 的数据：~8 rows (大约)
DELETE FROM `user`;
